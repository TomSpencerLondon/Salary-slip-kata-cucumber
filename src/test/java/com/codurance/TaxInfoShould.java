...
public class TaxInfoShould {
  ...
  public void calculate_zero_percent_tax_for_earnings_up_to_11000(BigDecimal annualGrossSalary,
                                                      BigDecimal taxFreeAllowance,
                                                      BigDecimal taxableIncome,
                                                      BigDecimal taxPayable) {
    TaxInfo taxInfo = new TaxInfo(annualGrossSalary);

    assertEquals(taxFreeAllowance, taxInfo.taxFreeAllowance());
    assertEquals(taxableIncome, taxInfo.taxableIncome());
    assertEquals(taxPayable, taxInfo.taxPayable());
  ...
  public void calculate_20_percent_tax_for_earnings_between_11000_and_43000(BigDecimal annualGrossSalary,
                                                                BigDecimal taxFreeAllowance,
                                                                BigDecimal taxableIncome,
                                                                BigDecimal taxPayable) {
    TaxInfo taxInfo = new TaxInfo(annualGrossSalary);
